...
**Every GraphQL document in this module has been executed against a real Shopify
development store on API 2025-07** - every read and every write, not just the test
stubs. Nothing here is only schema-checked. What that found:
...
| customer query (nested email/phone) | executed OK |
| order query including the customer block | executed OK |
| `customerCreate`, `customerUpdate` | executed OK, read back, then deleted |
| `fulfillmentCreate` with tracking | executed OK, order went FULFILLED, then cancelled |
